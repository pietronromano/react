...
References:
- https://www.udemy.com/course/modern-react-from-the-beginning/learn/lecture/51286245#overview
- 05-lifecycle-and-useeffect-hook/03-lifecycle-methods-classes.md

Starting in React 16.8, we can now use functional components to tap into these lifecycle methods.
Classes used to be the only way to create stateful components and access lifecycle methods.
Compare and contrast clases and components:
- Classes can have state and lifecycle methods, while functional components were originally stateless and did not have lifecycle methods.
...
- Classes use the `this` keyword to access props and state, while functional components do not use `this` and instead rely on hooks like `useState` and `useEffect`.
- Classes can be more verbose and require more boilerplate code, while functional components are often more concise and easier to read.
- Functional components with hooks can lead to better performance optimizations by React, as they can be more easily optimized by the React team.
- Overall, functional components with hooks are now the preferred way to create components in React, as they offer a simpler and more flexible approach to building React applications.


## To run in development mode:
```bash
cd my-modern-react/lessons/05-lifecycle-useeffect/50-classes/playground
npm install
npm run dev
```
...
<img src="../../../docs/images/react-lifecycle-2.png" width="600" />
